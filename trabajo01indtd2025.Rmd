...
criterio.Wald = function(tablaX,favorable=TRUE) { # favorable: indica si los valores son de beneficio (TRUE) o de coste (FALSE)

  X = tablaX;
  if (favorable) {
      # caso beneficio (Implementa el maximin)
    AltW = apply(X,MARGIN=1,min); # mínimo de cada fila (peor caso de cada alternativa)
    ##AltW
    Wald = max(AltW); # se elige el máximo de esos mínimos
    Alt_Wald = which.max.general(AltW); # índice(s) de la(s) alternativa(s) óptima(s)
    metodo = 'favorable';
  } else {
      # caso coste (Implementa el minimax)
    AltW = apply(X,MARGIN=1,max);  # máximo de cada fila (peor caso de cada alternativa)
    ##AltW
    Wald = min(AltW); # se elige el mínimo de esos máximos
    Alt_Wald = which.min.general(AltW); # índice(s) de la(s) alternativa(s) óptima(s)
    metodo = 'desfavorable';
  }
  resultados = list();
  resultados$criterio = 'Wald'; # nombre del criterio
  resultados$metodo = metodo; # favorable o desfavorable
  resultados$tablaX = tablaX; # matriz original
  resultados$ValorAlternativas = AltW; # valores Wald de cada alternativa
  resultados$ValorOptimo = Wald; # valor óptimo (máximo de los mínimos o mínimo de los máximos)
  resultados$AlternativaOptima = Alt_Wald; # alternativa(s) elegida(s)
...
criterio.Hurwicz = function(tablaX,alfa=0.3,favorable=TRUE) {
  ...
  if (favorable) {
    Altmin = apply(X,MARGIN=1,min); # peor resultado de cada alternativa
    Altmax= apply(X,MARGIN=1,max); # mejor resultado de cada alternativa
    AltH = alfa * Altmax + (1-alfa) * Altmin # combinación lineal
    Hurwicz = max(AltH) # se elige el máximo
    Alt_Hurwicz = which.max.general(AltH) # índice(s) de la(s) alternativa(s) óptima(s)
    metodo = 'favorable';
  }

  else {
    Altmin = apply(X,MARGIN=1,min); # mejor resultado posible (menor coste)
    Altmax= apply(X,MARGIN=1,max); # peor resultado posible (mayor coste)
    AltH = (1-alfa) * Altmax + alfa * Altmin # combinación inversa
    Hurwicz = min(AltH) # se elige el mínimo
    Alt_Hurwicz = which.min.general(AltH) # índice(s) de la(s) alternativa(s) óptima(s)
    metodo = 'desfavorable';
  }

  resultados = list();
  resultados$criterio = 'Hurwicz';
  resultados$alfa = alfa; # valor del factor de optimismo usado
  resultados$metodo = metodo;
  resultados$tablaX = tablaX; # matriz de decisión original
  resultados$ValorAlternativas = AltH; # valores Hurwicz (Hi=resultado de la fórmula) para cada alternativa
  resultados$ValorOptimo = Hurwicz; # valor óptimo (máximo o mínimo de los Hi)
  resultados$AlternativaOptima = Alt_Hurwicz; # índice(s) de la(s) alternativa(s) elegida(s)
...
criterio.Hurwicz.General = function(tablaX,alfa=0.3,favorable=TRUE) {
  ...
  if (favorable) {
    ...
    Altmax= apply(X,MARGIN=1,max); # mejor resultado de cada alternativa
    if (alfa<=1) {
      valfa = c(alfa); # usa un único valor
    } else {
      valfa = seq(from=0,to=1,by=(1/alfa)); # genera varios valores entre 0 y 1
    }
